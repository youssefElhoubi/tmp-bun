...
const scrapNewEgg = async (url: string): Promise<ProductInfo> => {
    ...
    try {
        ...
        const price: string = await page.$eval(
            ".price-new-right .price-current strong",
            (el) => (el as HTMLElement).innerText.trim()
        );

        const sub: string = await page.$eval(
            ".price-new-right .price-current sup",
            (el) => (el as HTMLElement).innerText.trim()
        );

        const holePrice: string = price + sub;
...
// Run directly from CLI (optional)
if (require.main === module) {
    const url = process.argv[2];
    if (url) {
        scrapNewEgg(url).then(result => console.log(JSON.stringify(result, null, 2)));
    }
}
